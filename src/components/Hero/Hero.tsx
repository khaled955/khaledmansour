...
interface Props {
  summary: string;
  socials: { linkedin: string };
  showcaseImages: string[]; // NEW
}

export default function Hero({ summary, socials, showcaseImages }: Props) {
  const reduce = useReducedMotion();
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16 sm:py-24 grid md:grid-cols-2 gap-10 items-center">
        ...
        >
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            <a
              href="#projects"
              className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-amber-500 text-white hover:bg-amber-600 focus-visible:ring-2 focus-visible:ring-amber-500"
            >
              View Projects
            </a>
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-neutral-300 dark:border-neutral-700 hover:bg-neutral-100 dark:hover:bg-neutral-800"
            >
              Contact Me
            </a>
          </div>
        ...
        {/* RIGHT SIDE: project showcase instead of a portrait */}
        <motion.div
        ...
        >
          <HeroShowCase images={showcaseImages} />
        </motion.div>
      </div>
    </section>
  );
